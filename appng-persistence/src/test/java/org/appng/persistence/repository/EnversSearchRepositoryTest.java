...
public class EnversSearchRepositoryTest {
	...
	public void test() {
		...
		Assert.assertNotNull(repo.findRevision(entityId, entityRevisions.get(0).getRequiredRevisionNumber()));
		Assert.assertEquals(1, repo.findRevisions(all.get(1).getId()).getContent().size());
		Assert.assertEquals(entityRevisions.get(entityRevisions.size() - 1).getRequiredRevisionNumber(),
				repo.findLastChangeRevision(entityId).get().getRequiredRevisionNumber());
	}
