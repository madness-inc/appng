...
+
The http://www.mojohaus.org/jaxb2-maven-plugin/Documentation/v2.2/[jaxb2-maven-plugin^] can be used to generate the Java classes using the https://eclipse-ee4j.github.io/jaxb-ri/[xjc^] binding compiler on this schema. The http://www.mojohaus.org/build-helper-maven-plugin/index.html[build-helper-maven-plugin^] is then used to add those classes to the project sources.
+
...
the {jee}/jakarta/servlet/ServletContext.html[jakarta.servlet.ServletContext^] appNG is executed in.
...
As the `PLATFORM` scope, it is bound to the {jee}/jakarta/servlet/ServletContext.html[jakarta.servlet.ServletContext^] appNG is executed in. There are no built-in attributes that
are stored in the `SITE` scope.


==== `SESSION` scope
This is propably the most commonly used scope when developing appNG applications. It is bound to the {jee}/jakarta/servlet/http/HttpSession.html[jakarta.servlet.http.HttpSession^] and
...
==== `REQUEST` scope
The `REQUEST` scope is bound to the {jee}/jakarta/servlet/ServletRequest.html[jakarta.servlet.ServletRequest^] and therefore can be used to track the state of the request.
